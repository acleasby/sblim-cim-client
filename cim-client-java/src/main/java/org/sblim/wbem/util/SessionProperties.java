 ...
 * 3078216    2010-09-29  blaschke-oss Fix for a null pointer exception in 1.3.9.1
 * 3160431    2011-01-18  blaschke-oss Need to support property file on AIX.
 */
...
public class SessionProperties extends Properties implements Cloneable {
	...
	private static final String UNIX_SBLIM_CIM_CLIENT_PROPERTIES = "/etc/java/sblim-cim-client.properties"; //$NON-NLS-1$
	...
	private static String detectDefaultJsseProvider() {
		...
		if (providers != null && providers.length > 0) { return providers[0].getClass().getName(); }
	...
	public boolean loadProperties() {	
		...
		try {
			...
			if (System.getProperty("os.name").equalsIgnoreCase("Linux")
					|| System.getProperty("os.name").equalsIgnoreCase("AIX")) {
				file = new File(UNIX_SBLIM_CIM_CLIENT_PROPERTIES).getAbsoluteFile();
				if (file.exists()) {
					load(new FileInputStream(file));
					propertyFile = UNIX_SBLIM_CIM_CLIENT_PROPERTIES;
					return true;
				}
			}
